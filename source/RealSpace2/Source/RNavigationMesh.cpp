...
bool RNavigationMesh::Open(const char* szFileName, MZFileSystem* pZFileSystem)
{
	...
	// vertex -------------
	pFile->read(sizeof(int), &nVertCount);
	InitVertices(nVertCount);
	for (int i = 0; i < nVertCount; i++)
	{
		pFile->read(sizeof(rvector), &m_vertices[i]);
	}

	// face ---------------
	pFile->read(sizeof(int), &nFaceCount);
	InitFaces(nFaceCount);
	for (int i = 0; i < nFaceCount; i++)
	{
		pFile->read(sizeof(RNavFace), &m_faces[i]);
	}
	...
	for (RNodeArray::iterator itor = m_NodeArray.begin(); itor != m_NodeArray.end(); ++itor)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			if (pFile->read(sizeof(int), &nSideIndex))
			{
				if (nSideIndex >= 0)
				{
