...
bool RMesh::ReadXml(char* filename)
{
	...
	if(!XmlDoc.LoadFromMemory(buffer.get()))
	{
		return false;
	}
    pFile.reset();
...
bool RMesh::ReadNewElu(IFile *pFile, char* fname) {
#define MZF_READ(x,y) { if(pFile->read((y),(x)) != (y)) return false; }
#define MZF_READ_XOR(x,y) { if(pFile->read((y),(x)) != (y)) return false; }
#define MZF_READ_XOR_SPECIAL(x,y) { if(pFile->read((y),(x)) != (y)) return false; for (int i = 0; i < y; ++i) *(BYTE*)(x + i) ^= 0; }
  int i;
...
bool RMesh::ReadOldElu(IFile * pFile, ex_hd_t* m_phd_t) {
...
#define MZF_READ(x,y) { if(pFile->read((y),(x)) != (y)) return false; }
    ...
    std::unique_ptr<IFile> pFile = std::unique_ptr<IFile>(getFileManager()->open(fname));
    if (!pFile.get())
    {
	...
	if(t_hd.mesh_num != -1 && t_hd.mtrl_num != -1) {
		if(!ReadOldElu(pFile.get(), &t_hd))
        {
#ifdef RELEASE_DEBUG
            mlog("Read elu failed: %s\n", fname);
#endif
			return false;
        }
	}
