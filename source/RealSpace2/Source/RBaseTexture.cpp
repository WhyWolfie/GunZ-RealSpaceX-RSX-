...
bool RBaseTexture::OnRestore(bool bManaged)
{
	...
	if(m_pTextureFileBuffer == NULL)
    {
        ...
        pFile = std::unique_ptr<IFile>(getFileManager()->open(ddstexturefile));
        if (!pFile.get())
#endif
        {
            pFile = std::unique_ptr<IFile>(getFileManager()->open(m_szTextureName));
            if (!pFile.get())
            {
                __EP(2012);
                __EP(2010);
        ...
        m_pTextureFileBuffer = new char[m_nFileSize];

        __EP(2013);
        __BP(2014,"RBaseTexture::mzf.Read ");

        if (!pFile->read(m_nFileSize, m_pTextureFileBuffer))
            return false;
