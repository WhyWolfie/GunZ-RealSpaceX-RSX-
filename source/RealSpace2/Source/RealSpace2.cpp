...
D3DMULTISAMPLE_TYPE getAntiAliasing(bool fs);
...
bool RInitDisplay(HWND hWnd, const RMODEPARAMS *params)
{
	...
	g_d3dpp.MultiSampleType = g_MultiSample = getAntiAliasing(params->bFullScreen);
...
#	define _NVPERFHUD
    ...
    rsx::getEngine()->initialize(g_pd3dDevice);
...
void RResetDevice(const RMODEPARAMS *params)
{
    ...
    rsx::getEngine()->onLost();

	HRESULT hr=g_pd3dDevice->Reset(&g_d3dpp);

    rsx::getEngine()->onReset();
...
LPDIRECT3DSURFACE9 RCreateImageSurface(const char *filename)
{
	...
	std::unique_ptr<char> buffer = std::unique_ptr<char>(new char[pFile->getSize()]);
...
//	RGetDevice()->CreateImageSurface(1,1,D3DFMT_A8R8G8B8,&pSurface);
	...
	D3DXLoadSurfaceFromFileInMemory(pSurface,NULL,NULL,buffer.get(),pFile->getSize(),NULL,D3DX_FILTER_NONE,0,&info);
	pSurface->Release();
...
//	hr = RGetDevice()->CreateImageSurface(info.Width,info.Height,info.Format,&pSurface);
	...
	_ASSERT(hr==D3D_OK);
	hr = D3DXLoadSurfaceFromFileInMemory(pSurface,NULL,NULL,buffer.get(),pFile->getSize(),NULL,D3DX_FILTER_NONE,0,&info);
	_ASSERT(hr==D3D_OK);
...
{
    MAA_TYPE val;
    val.uiVal = 1 << nSettingAA;
    val.uiVal = val.uiVal == 1 ? 0 : val.uiVal;
    val.uiVal = val.uiVal > 16 ? 16 : val.uiVal;

    while (FAILED(g_pD3D->CheckDeviceMultiSampleType(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL , g_PixelFormat, fs, val.eVal, NULL )))
    {
	    if (val.uiVal == 0)
            break;
        else 
