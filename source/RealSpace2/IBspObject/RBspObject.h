...
class RBspObject : public IBspObject
{ 
public:
	...
	bool Open(const char *, const char* descExtension, ROpenFlag nOpenFlag, RFPROGRESSCALLBACK pfnProgressCallback, void *CallbackParam);

	bool Open(const char * fileName, const char * ext) {
		return Open(fileName, ext, ROF_RUNTIME, NULL, NULL);
	}

	bool OpenDescription(const char *);				// .xml 
	bool OpenRs(const char *);						// .rs  
	bool OpenBsp(const char *);						// .bsp
	bool OpenLightmap();							// .lm
//	bool OpenPathNode(const char *);				// .pat
	bool OpenCol(const char *);						// .col
	bool OpenNav(const char *);						// .nav
	...
	RBSPMATERIAL *GetMaterial(RSBspNode *pNode,int nIndex)		{ if(pNode) return GetMaterial(pNode->pInfo[nIndex].nMaterial);
																   else return GetMaterial(0);}
...
public:
	...
	bool ReadString(IFile *pfile,char *buffer,int nBufferSize);
	bool Open_Nodes(RSBspNode *pNode, IFile *pfile);
	bool Open_ColNodes(RSolidBspNode *pNode, IFile *pfile);
	...
	bool Open_ConvexPolygons(IFile *pfile);
