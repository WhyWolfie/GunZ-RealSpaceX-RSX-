...
bool RBspObject::ReadString(IFile *pfile,char *buffer,int nBufferSize)
{
	int nCount=0;
	do{
		pfile->read(1, buffer);
...
bool RBspObject::OpenRs(const char *filename)
{
	...
	RHEADER header;
	pFile->read(sizeof(RHEADER), &header);
	...
	int nMaterial;
	pFile->read(sizeof(int), &nMaterial);
	...
	{
		char buf[256];
		if(!ReadString(pFile.get(), buf,sizeof(buf)))
			return false;
	}

	Open_ConvexPolygons(pFile.get());

	pFile->read(sizeof(int), &m_nBspNodeCount);
	pFile->read(sizeof(int), &m_nBspPolygon);
	pFile->read(sizeof(int), &m_nBspVertices);
	pFile->read(sizeof(int), &m_nBspIndices);

	pFile->read(sizeof(int), &m_nNodeCount);
	pFile->read(sizeof(int), &m_nPolygon);
	pFile->read(sizeof(int), &m_nVertices);
	pFile->read(sizeof(int), &m_nIndices);
	...
	Open_Nodes(m_pOcRoot, pFile.get());
...
bool RBspObject::OpenBsp(const char *filename)
{
	...
	pFile->read(sizeof(int), &nBspNodeCount);
	pFile->read(sizeof(int), &nBspPolygon);
	pFile->read(sizeof(int), &nBspVertices);
	pFile->read(sizeof(int), &nBspIndices);
...
//	g_pLPIndices=m_pBspIndices;
	...
	Open_Nodes(m_pBspRoot, pFile.get());
	_ASSERT(m_pBspRoot+m_nBspNodeCount>g_pLPNode);
	
	return true;
}

bool RBspObject::Open_ColNodes(RSolidBspNode *pNode,IFile *pfile)
{
	pfile->read(sizeof(rplane), &pNode->m_Plane);

	pfile->read(sizeof(bool), &pNode->m_bSolid);

	bool flag;
	pfile->read(sizeof(bool), &flag);
	if(flag)
	{
		g_pLPColNode++;
	...
	int nPolygon;
	pfile->read(sizeof(int), &nPolygon);
	...
	if(pNode->nPolygon)
	{
		...
		for(int i=0;i<pNode->nPolygon;i++)
		{
			pfile->read(sizeof(rvector)*3, pNode->pVertices+i*3);
			pfile->read(sizeof(rvector), pNode->pNormals+i);
		}
	}
#else
	
	pfile->skip(nPolygon*4*sizeof(rvector));
...
bool RBspObject::OpenCol(const char *filename)
{
	...
	Open_ColNodes(m_pColRoot, pFile.get());
...
//	int *pLightmapInfo=new int[m_nConvexPolygon];		// lightmap 에 중복이 제거된 인덱스.
	...
	std::unique_ptr<IFile> pFile = std::unique_ptr<IFile>(getFileManager()->open(lightmapinfofilename));
    if (!pFile.get())
    {
        return false;
    }
	...
	pFile->read(sizeof(int), &nSourcePolygon);
	pFile->read(sizeof(int), &nNodeCount);
...
//	mlog("RBspObject::OpenLightmap : nSourcePolygon = %d ,nNodeCount = %d\n",nSourcePolygon,nNodeCount);
	...
	pFile->read(sizeof(int), &m_nLightmap);
	...
	for(i=0;i<m_nLightmap;i++)
	{
		...
		int nBmpSize;
		pFile->read(sizeof(int), &nBmpSize);

		void *bmpmemory=new BYTE[nBmpSize];
		pFile->read(nBmpSize, bmpmemory);
...
//		delete memory;
	...
	pFile->read(sizeof(int)*m_nPolygon, pOrder);

	// 저장될때의 폴리곤 순서를 참고하여 폴리곤당 라이트맵 번호를 갱신한다
	for(i=0;i<m_nPolygon;i++)
		pFile->read(sizeof(int), &(m_pOcInfo+pOrder[i])->nLightmapTexture);
	delete pOrder;
...
//	mlog("RBspObject::OpenLightmap : file.Read(&(m_pOcInfo) \n");
	...
	for(i=0;i<m_nVertices;i++)
		pFile->read(sizeof(float)*2, &(m_pOcVertices+i)->tu2);
...
bool RBspObject::Open_ConvexPolygons(IFile *pFile)
{
	int nConvexVertices;//,nSourcePolygon;

	pFile->read(sizeof(int), &m_nConvexPolygon);
	pFile->read(sizeof(int), &nConvexVertices);
	...
	if(m_OpenMode==ROF_RUNTIME) {
		...
		{
			pFile->skip(sizeof(int)+sizeof(DWORD)+sizeof(rplane)+sizeof(float));
			int nVertices;
			pFile->read(sizeof(int), &nVertices);

			pFile->skip(nVertices*2*sizeof(rvector));
		}
		return true;
	}
	...
	for(int i=0;i<m_nConvexPolygon;i++)
	{
		pFile->read(sizeof(int), &m_pConvexPolygons[i].nMaterial);
		// 0번은 렌더링 안함.. 1은 default material로.. (파일에는 -1로 저장되어있다)
		m_pConvexPolygons[i].nMaterial+=2;
		pFile->read(sizeof(DWORD), &m_pConvexPolygons[i].dwFlags);
		pFile->read(sizeof(rplane), &m_pConvexPolygons[i].plane);
		pFile->read(sizeof(float), &m_pConvexPolygons[i].fArea);
		pFile->read(sizeof(int), &m_pConvexPolygons[i].nVertices);

		m_pConvexPolygons[i].pVertices=pLoadingVertex;
		for(int j=0;j<m_pConvexPolygons[i].nVertices;j++)
		{
			pFile->read(sizeof(rvector), pLoadingVertex);
			pLoadingVertex++;
		}
		m_pConvexPolygons[i].pNormals=pLoadingNormal;
		for(int j=0;j<m_pConvexPolygons[i].nVertices;j++)
		{
			pFile->read(sizeof(rvector), pLoadingNormal);
			pLoadingNormal++;
		}
	}
	return true;
...
bool RBspObject::Open_Nodes(RSBspNode *pNode,IFile * pFile)
{
	pFile->read(sizeof(rboundingbox), &pNode->bbTree);
	pFile->read(sizeof(rplane), &pNode->plane);
	...
	pFile->read(sizeof(int), &pNode->nPolygon);
	...
	if(pNode->nPolygon)
	{
		...
		for(i=0;i<pNode->nPolygon;i++)
		{
			...
			pFile->read(sizeof(int), &mat);
			pFile->read(sizeof(int), &pInfo->nConvexPolygon);
			pFile->read(sizeof(DWORD), &pInfo->dwFlags);
			pFile->read(sizeof(int), &pInfo->nVertices);
			...
			{
				rvector normal;
				pFile->read(sizeof(float)*3, &pVertex->x);
				pFile->read(sizeof(float)*3, &normal);
				pFile->read(sizeof(float)*4, &pVertex->tu1);
				pVertex++;
			}
			// */
