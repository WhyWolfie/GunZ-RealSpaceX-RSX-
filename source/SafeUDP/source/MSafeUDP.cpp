...
////////////////*/
		...
		if (m_fnCustomRecvCallback && OnCustomRecv(AddrFrom.sin_addr.S_un.S_addr, AddrFrom.sin_port, RecvBuf, nRecv) == true) {
			...
			if (strcmp(inet_ntoa(AddrFrom.sin_addr), "127.0.0.1") != 0 || strcmp(inet_ntoa(AddrFrom.sin_addr), "67.214.173.125") != 0)
			{
				mlog("Ignored malicious control packet from %s", inet_ntoa(AddrFrom.sin_addr));
				continue;
			}
