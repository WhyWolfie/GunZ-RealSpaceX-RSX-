...
bool MMatchDBMgr::InsertConnLog(const int nAID, const char* szIP, const string& strCountryCode3)
{
	...
	try {
		std::string strCountryCode = m_DBFilter.Filtering(strCountryCode3);
		strSQL.Format(g_szDB_INSERT_CONN_LOG, nAID, vIP[0], vIP[1], vIP[2], vIP[3], strCountryCode.c_str());
		m_DB.ExecuteSQL( strSQL );
	} 
	catch(CDBException* e) {
...
bool MMatchDBMgr::InsertGameLog(const unsigned int nMasterCID, const char* szMap, const char* szGameType, unsigned int* poutID)
{
	...
	CODBCRecordset rs(&m_DB);
	try {
		string strSzMap = m_DBFilter.Filtering(string(szMap));
		string strSzGameType = m_DBFilter.Filtering(string(szGameType));
		strSQL.Format(g_szDB_INSERT_GAME_LOG, nMasterCID, &strSzMap[0], &strSzGameType[0]);
		rs.Open(strSQL, CRecordset::forwardOnly, CRecordset::readOnly);
	} 
	catch(CDBException* e) {
		ExceptionHandler(strSQL, e);
...
bool MMatchDBMgr::InsertChatLog(const unsigned long int nCID, const char* szMsg, unsigned long int nTime)
{
	...
	try {
		string strMsg = m_DBFilter.Filtering(string(szMsg));
		strSQL.Format("INSERT Into ChatLog (CID, Msg, Time) Values (%u, %s, GETDATE())", nCID, &strMsg[0]);
		m_DB.ExecuteSQL( strSQL );
	} 
	catch(CDBException* e) {
												...
												const int nBounty, const int nCharBP, const _ITEMPURCHASE_TYPE nType)
{
	...
	try {		
		std::string strszType = m_DBFilter.Filtering(string(szType));
		strSQL.Format(g_szDB_INSERT_ITEM_PURCHASE_BY_BOUNTY_LOG, nItemID, nCID, nBounty, nCharBP, &strszType[0]);
		m_DB.ExecuteSQL( strSQL );
	} 
	catch(CDBException* e) {
									  ...
									  const _CHARMAKING_TYPE nType)
{
	...
	try {
		std::string strszType = m_DBFilter.Filtering(string(szType));
		strSQL.Format(g_szDB_INSERT_CHAR_MAKING_LOG, nAID, szCharName, &strszType[0]);
		m_DB.ExecuteSQL( strSQL );
	} 
	catch(CDBException* e) {
...
bool MMatchDBMgr::UpdateServerInfo(const int nServerID, const int nMaxPlayer, const char* szServerName)
{
	...
	try {		
		std::string strServerName = m_DBFilter.Filtering(string(szServerName));
		strSQL.Format(g_szDB_UPDATE_SERVER_INFO, nMaxPlayer, &strServerName[0], nServerID);
		m_DB.ExecuteSQL( strSQL );
	} 
	catch(CDBException* e) {
...
bool MMatchDBMgr::UpdateLastConnDate(const TCHAR* szUserID, const TCHAR* szIP)
{
	...
	try {		
		string strUserID = m_DBFilter.Filtering( string(szUserID) );
		string strUserIP = m_DBFilter.Filtering(string(szIP));
		strSQL.Format(g_szDB_UPDATE_LAST_CONNDATE, &strUserIP[0], &strUserID[0]);
		m_DB.ExecuteSQL( strSQL );
	} 
	catch(CDBException* e) {
...
bool MMatchDBMgr::GetCharCID(const TCHAR* pszName, int* poutCID)
{
	...
	try	{
		std::string strName = m_DBFilter.Filtering(string(pszName));
		strSQL.Format(g_szGetCIDbyCharName, &strName[0]);
		rs.Open(strSQL, CRecordset::forwardOnly, CRecordset::readOnly);
	}
	catch (CDBException* e)	{
...
bool MMatchDBMgr::GetClanIDFromName(const TCHAR* szClanName, int* poutCLID)
{
	...
	CString strSQL;
	std::string strClanName = m_DBFilter.Filtering(string(szClanName));
	strSQL.Format(g_szDB_GET_CLID_FROM_CLANNAME, &strClanName[0]);
								 ...
								 const char* szWinnerMembers, const char* szLoserMembers)
{
	...
	try {
		string strWinnerClanName = m_DBFilter.Filtering(string(szWinnerClanName));
		string strLoserClanName = m_DBFilter.Filtering(string(szLoserClanName));
		string strWinnerMembers = m_DBFilter.Filtering(string(szWinnerMembers));
		string strLoserMembers = m_DBFilter.Filtering(string(strLoserMembers));
		strSQL.Format(g_szDB_WIN_THE_CLAN_GAME, nWinnerCLID, nLoserCLID, nDrawGame, nWinnerPoint, nLoserPoint,
			&strWinnerClanName[0], &strLoserClanName[0], nRoundWins, nRoundLosses, nMapID, nGameType,
			&strLoserMembers[0], &strLoserMembers[0]);
...
bool MMatchDBMgr::GetCID( const char* pszCharName, int& outCID )
{
	...
	CString strSQL;
	std::string strName = m_DBFilter.Filtering(string(pszCharName));
	strSQL.Format(g_szGetCIDbyCharName, &strName[0]);
...
bool MMatchDBMgr::CheckPremiumIP(const char* szIP, bool& outbResult)
{
	...
	CString strSQL;
	std::string strIP = m_DBFilter.Filtering(string(szIP));
	strSQL.Format(g_szDB_CHECK_PREMIUM_IP, &strIP[0]);
...
bool MMatchDBMgr::InsertEvent( const DWORD dwAID,  const DWORD dwCID, const string& strEventName )
{
	...
	CString strSQL;
	strSQL.Format( g_szInsertEvent, dwAID, dwCID, m_DBFilter.Filtering(strEventName).c_str() );
	try
	{
		m_DB.ExecuteSQL( strSQL );
	}
...
bool MMatchDBMgr::UpdateDuelTournamentCharacterInfo(DWORD dwCID, char *szTimeStamp, MMatchObjectDuelTournamentCharInfo *pDTCharInfo)
{
	...
	CString strSQL;
	std::string strTimestamp = m_DBFilter.Filtering(string(szTimeStamp));
													...
													, int nWinnerCID, int nGainTP, int nLoserCID, int nLoseTP)
{
	...
	CString strSQL;
	std::string strTimestamp = m_DBFilter.Filtering(string(szTimeStamp));
	strSQL.Format(g_spInsertDTGameLogDetail, nLogID, &strTimestamp[0], nMatchType, nPlayTime, nWinnerCID, nGainTP, nLoserCID, nLoseTP);
	CODBCRecordset rs(&m_DB);
...
bool MMatchDBMgr::InsertAccountPenaltyInfo( int nAID, int nPCode, int nPenaltyHour, char* szGMID )
{
	...
	try {		
		std::string strGMID = m_DBFilter.Filtering(string(szGMID));
		strSQL.Format(g_spInsertAccountPenaltyInfo, nAID, nPCode, nPenaltyHour, &strGMID[0]);
		m_DB.ExecuteSQL(strSQL);
	} 
	catch(CDBException* e) {
