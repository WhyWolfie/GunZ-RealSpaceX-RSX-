...
bool ZSoundEngine::LoadResource(char* pFileName)
{
	...
	std::unique_ptr<IFile> pFile = std::unique_ptr<IFile>(getFileManager()->open(pFileName));
    if (!pFile.get())
    {
        Data.Destroy();
        return;
	...
	if(!Data.LoadFromMemory(buffer.get()))
	{
		Data.Destroy();
		return;
	}
...
bool ZSoundEngine::OpenMusic(const char* szFileName, MZFileSystem* pfs)
{
    ...
    m_pMusicBuffer = new char[pFile->getSize()+1];
    pFile->read(pFile->getSize(), m_pMusicBuffer);
    m_pMusicBuffer[pFile->getSize()] = 0;

    return ZGetSoundFMod()->OpenStream( m_pMusicBuffer, pFile->getSize() );
}
...
bool ZSoundEngine::LoadResource( char* pFileName_ ,ZLoadingProgress *pLoading )
{
	...
	std::unique_ptr<IFile> pFile = std::unique_ptr<IFile>(getFileManager()->open(pFileName_));
    if (!pFile.get())
    {
        Data.Destroy();
        return false;
