...
#define NEXTLINE() { sy+=15; }
	...
	if(m_nShowDebugInfo==2) {
		...
		sprintf(buffer, "mapobject = %d , frustum culled = %d , occlusion culled = %d",
			ZGetGame()->GetWorld()->GetBsp()->GetMapObjectList()->size(),/*ZGetGame()->GetWorld()->GetBsp()->GetDebugInfo()->nMapObjectFrustumCulled*/-1,/*ZGetGame()->GetWorld()->GetBsp()->GetDebugInfo()->nMapObjectOcclusionCulled*/-1);
		OUTTEXT();
		NEXTLINE();
...
//	if(!g_pGame->m_pMyCharacter->m_bLand) strcat(buffer,"jump ");
	...
	*/
	...
	if(ZApplication::GetGameInterface()->IsCursorEnable())
	{
		...
		bool bPicked= false;//ZGetGame()->GetWorld()->GetBsp()->PickOcTree(pos,dir,&bpi);
		if(bPicked)
		{
