...
void ZGameInterface::LoadBitmaps(const char* szDir, const char* szSubDir, ZLoadingProgress *pLoadingProgress)
{
    ...
    uint ext_bmp = 0x706D622E, // ".bmp"
         ext_png = 0x676E702E, // ".png"
         ext_tga = 0x6167742E; // ".tga"

    std::vector<const char*> fileNames;
    const char * fileName = getFileManager()->getFirstFileName();

    while (fileName != nullptr)
    {
        uint nLen = strlen(fileName);
        if (nLen >= 4)
        {
            const uint ext = *(uint*)(fileName + nLen - 4);
            if (ext == ext_bmp || ext == ext_png || ext == ext_tga)
                fileNames.push_back(fileName);
        }

        fileName = getFileManager()->getNextFileName();
    }

    uint size = fileNames.size();
    for (uint i = 0; i < size; ++i)
    {
        if(pLoadingProgress && i%10==0)
            pLoadingProgress->UpdateAndDraw((float)i/(float)size);


        char aliasname[256];
        char drive[_MAX_DRIVE], dir[_MAX_DIR], fname[_MAX_FNAME], ext[_MAX_EXT];
        _splitpath(fileNames[i], drive, dir, fname, ext);

        sprintf(aliasname, "%s%s", fname, ext);


        std::unique_ptr<MBitmapR2> pBitmap = std::unique_ptr<MBitmapR2>(new MBitmapR2);
        if(pBitmap->Create(aliasname, RGetDevice(), fileNames[i]) == true)
            MBitmapManager::Add(pBitmap.release());
...
#define DISABLE_HACK_BYPASS
void ZGameInterface::OnStageCreate(void)
{
#ifndef DISABLE_HACK_BYPASS
	// WPE hacking protect
	HMODULE hMod = GetModuleHandle( "ws2_32.dll"); 
	FARPROC RetVal = GetProcAddress( hMod, "recv"); 
...
void ZGameInterface::ChangeToCharSelection(void)
{
	...
	ZPostAccountCharList();		// update sgk 0409 // 캐릭터 리스트 요청
}
